...
#pragma omp parallel for num_threads(nparallel)
  ...
  auto f = [&](const std::vector<T2>& vs) {
    ...
    for(Idx k=0; k<vs.size(); k++) {
      ...
      for(Idx if1=0; if1<dual.NVertices(); if1++){
        ...
        T12_mean[b0_g.first] += s.T12(if1, ising);
      }

      for(Idx g=0; g<NIh; g++){
        T1 tmp = zero;
        for(Idx i=0; i<dual.NVertices(); i++) tmp[ i ] = s(orbits[i0][g])*s(orbits[j0][g])*s.T12( orbits[i][g], ising );
        mean += tmp;
      }
    }
    mean /= vs.size() * NIh;
    ...
    for(Idx b0=0; b0<orbits.nbase(); b0++) {
      Nsq_mean[b0] /= vs.size() * orbits.npts[b0];
      T12_mean[b0] /= vs.size() * orbits.npts[b0];
    }
    const Idx b0 = orbits.b0_g_pairs[i0].first;
    const double norm = Nsq_mean[b0]; // @@@
    for(Idx i=0; i<dual.NVertices(); i++){
      const auto& b0_g = orbits.b0_g_pairs[i];
      mean[i] -= norm * T12_mean[b0_g.first];
      mean[i] /= norm;
    }
  ...
  for(ibin_min=0; ibin_min<obs.nbins; ibin_min++){
    // check ckpoints
    const std::string filepath = obsdir+"ssT12_"+std::to_string(ibin_min)+".dat";
...
#pragma omp parallel for num_threads(nparallel)
  ...
  for(int ibin=ibin_min; ibin<obs.nbins; ibin++){
    ...
    const std::string filepath = obsdir+"ssT12_"+std::to_string(ibin)+".dat";
