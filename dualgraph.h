    ...
    for(Idx if1=0; if1<dual.NVertices(); if1++){
      ...
      for(int df0=0; df0<3; df0++){
        for(int df2=0; df2<3; df2++){
          ...
          const double al10 = spin.alphas[dual.directedlinkidx(if1, df0)];
          const double al12 = spin.alphas[dual.directedlinkidx(if1, df2)];
          double dalpha = (al10+M_PI)-al12;
          while(dalpha>M_PI) dalpha -= 2.0*M_PI;
          while(dalpha<-M_PI) dalpha += 2.0*M_PI;

          double u=1.0;
          u *= std::sqrt(D.kappas[dual.directedlinkidx(if1,df2)]);
          u *= std::sqrt(D.kappas[dual.directedlinkidx(if1,df0)]);
          u /= D.mus[if1];
          u *= std::cos( dalpha/2 );
