...
// int main(){
  ...
  { // this will only modify q loop
    ...
    std::cout << "trTs: " << std::endl;
    for(Idx if0=0; if0<10; if0++){
      // double mu = D.mus[if0];
      // double vol = dual.site_volumes[if0];
      std::cout << s.s.trT(if0, ising) << std::endl;
      std::cout << 2.0*M_PI / (ising.dual.site_volumes[if0]) << std::endl;
      // std::cout << mu << " " << mu/vol << std::endl;
    }
    return 1;
    ...
    for(int mu=0; mu<3; mu++) {
      ...
      Ts[mu] = ising.dbeta_dkappas[ell] - eps_coeff;
    }
    ...
    //   std::cout << "trT " << s.s.trT(0, ising) << std::endl;
    //   // sum2 += K*std::exp(s.S());
