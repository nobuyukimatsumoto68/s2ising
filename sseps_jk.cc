...
#pragma omp parallel for num_threads(nparallel)
  ...
  auto f = [&](const std::vector<T2>& vs) {
    ...
    std::vector<double> eps_mean(orbits.nbase(), 0.0);
    ...
    for(Idx k=0; k<vs.size(); k++) {
      ...
      for(Idx g=0; g<NIh; g++){
        T1 tmp = zero;
        for(Idx i=0; i<dual.NVertices(); i++) tmp[ i ] = s(orbits[i0][g])*s(orbits[j0][g])*s.eps_hat( orbits[i][g], ising );
        mean += tmp;
      }
    }
    mean /= vs.size() * NIh;
    ...
    for(Idx b0=0; b0<orbits.nbase(); b0++) {
      ...
      eps_mean[b0] /= vs.size() * orbits.npts[b0];
    }
    ...
    for(Idx i=0; i<dual.NVertices(); i++){
      ...
      mean[i] -= norm * eps_mean[b0_g.first];
      mean[i] /= norm;
    }
  ...
  for(ibin_min=0; ibin_min<obs.nbins; ibin_min++){
    // check ckpoints
    const std::string filepath = obsdir+"sseps_"+std::to_string(ibin_min)+".dat";
...
#pragma omp parallel for num_threads(nparallel)
  ...
  for(int ibin=ibin_min; ibin<obs.nbins; ibin++){
    ...
    const std::string filepath = obsdir+"sseps_"+std::to_string(ibin)+".dat";
